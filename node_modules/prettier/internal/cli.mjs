  ...
  "node_modules/diff/lib/patch/create.js"(exports) {
    ...
    function structuredPatch(oldFileName, newFileName, oldStr, newStr, oldHeader, newHeader, options2) {
      ...
      var diff2 = (
        ...
        (0, /*istanbul ignore end*/
        /*istanbul ignore start*/
        _line.diffLines)(oldStr, newStr, options2)
      );
      if (!diff2) {
        return;
      }
          ...
          if (oldRangeStart) {
            if (lines.length <= options2.context * 2 && i2 < diff2.length - 2) {
              ...
              if (i2 >= diff2.length - 2 && lines.length <= options2.context) {
                var old***REMOVED***Newline = /\n$/.test(oldStr);
                var new***REMOVED***Newline = /\n$/.test(newStr);
                var noNlBeforeAdds = lines.length == 0 && curRange.length > hunk.oldLines;
                if (!old***REMOVED***Newline && noNlBeforeAdds && oldStr.length > 0) {
                  curRange.splice(hunk.oldLines, 0, "\\ No newline at end of file");
                }
                if (!old***REMOVED***Newline && !noNlBeforeAdds || !new***REMOVED***Newline) {
  ...
  "node_modules/glob/common.js"(exports) {
    ...
    function setopts(self, pattern, options2) {
      ...
      if (!ownProp(options2, "cwd"))
        ...
        self.cwd = path10.resolve(options2.cwd);
        self.changedCwd = self.cwd !== cwd2;
      }
      self.root = options2.root || path10.resolve(self.cwd, "/");
  ...
  "node_modules/rimraf/rimraf.js"(exports, module) {
    ...
    var rmkids = (p, options2, cb) => {
      ...
      options2.readdir(p, (er, files) => {
        ...
        let errState;
        files.forEach((f) => {
          rimraf(path10.join(p, f), options2, (er2) => {
            if (errState)
              return;
            if (er2)
    ...
    var rmkidsSync = (p, options2) => {
      assert(p);
      assert(options2);
      options2.readdirSync(p).forEach((f) => rimrafSync(path10.join(p, f), options2));
...
function getOptionsWithOpposites(options2) {
  const optionsWithOpposites = options2.map((option) => [option.description ? option : null, option.oppositeDescription ? {
    ...option,
    name: `no-${option.name}`,
    type: "boolean",
...
async function* expandPatterns(context) {
  ...
  for await (const pathOrError of expandPatternsInternal(context)) {
    ...
    const relativePath = path2.relative(cwd2, pathOrError);
...
async function getOptionsForFile(context, filepath) {
  ...
  const hasPlugins = options2 == null ? void 0 : options2.plugins;
  if (hasPlugins) {
...
function findUpMultipleSync(name, options2 = {}) {
  let directory = path4.resolve(toPath2(options2.cwd) || "");
  const { root } = path4.parse(directory);
  const stopAt = options2.stopAt || root;
...
function findCacheDirectory(options2 = {}) {
  if (env2.CACHE_DIR && !["true", "false", "1", "0"].includes(env2.CACHE_DIR)) {
    return useDirectory(path6.join(env2.CACHE_DIR, options2.name), options2);
  }
  let { cwd: directory = cwd() } = options2;
  if (options2.files) {
    directory = (0, import_common_path_prefix.default)(options2.files.map((file) => path6.resolve(directory, file)));
  }
  ...
  return useDirectory(path6.join(directory, "node_modules", ".cache", options2.name), options2);
}
...
async function formatStdin(context) {
  ...
  try {
    ...
    const options2 = await get_options_for_file_default(
      context,
...
${error.message}`
    ...
    if (context.argv.write) {
      if (isDifferent) {
        ...
        try {
          await fs5.writeFile(filename, output, "utf8");
          shouldSetCache = true;
        } catch (error) {
          context.logger.error(
...
${error.message}`
        ...
        const message = `${source_default.grey(normalizeToPosix(filename))} ${Date.now() - start}ms`;
        if (isCacheExists) {
